...
if __name__ == '__main__':
    ...
    atm_strike = round(getCMP('NSE:BANK NIFTY'), -2)

    next_thursday_expiry = datetime.today() + relativedelta(weekday=TH(1))

    symbol_ce = get_symbols(next_thursday_expiry.date(), 'BANK NIFTY', atm_strike+100, 'CE')
    symbol_pe = get_symbols(next_thursday_expiry.date(), 'BANK NIFTY', atm_strike-100, 'PE')
